...
-(void)initView{
    ...
    _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(25, 35, self.bounds.size.width-50, 120)];
    ...
    selectViewBac.backgroundColor = [UIColor colorWithRed:255/255.f green:255/255.f blue:255/255.f alpha:0.2];
    ...
    UIButton * cancleBtn = [[UIButton alloc] initWithFrame:CGRectMake(30, CGRectGetMaxY(_pickerView.frame)+5, 90, 30)];
    ...
    UIButton * sureBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.bounds.size.width -120, CGRectGetMaxY(_pickerView.frame) +5, 90, 30)];
    ...
    [cancleBtn addTarget:self action:@selector(clickCancleBtnAction) forControlEvents:UIControlEventTouchUpInside];
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    if (component==0) {
        text.text = [NSString stringWithFormat:@"%@年",[_yearArray objectAtIndex:row]];
    }
    if (component==1) {
        text.text = [NSString stringWithFormat:@"%@月",[_mothArray objectAtIndex:row]];
    }
    if (component==2) {
        text.text = [NSString stringWithFormat:@"%@日",[_dayArray objectAtIndex:row]];
    }
...
- (void)clickSureBtnAction {
    [self.delegate btnindex:self.model show:YES];
}

- (void)clickCancleBtnAction {
    [self.delegate btnindex:nil show:NO];
}
